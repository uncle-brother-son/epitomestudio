...
export function PageTransition({ children }: { children: ReactNode }) {
  ...
  // Navigate function for programmatic navigation with transitions
  const navigate = (url: string, delay: number = 0) => {
    setTimeout(() => {
  ...
  const contextValue = useMemo(() => ({ 
    isTransitioning: isWaitingForContent,
    navigate
  }), [isWaitingForContent]);
  ...
  // FLOW 2: Intercept link clicks - fade out, navigate, fade in
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      if (link && link.href && !link.target && link.href.startsWith(window.location.origin)) {
        ...
        // Don't intercept clicks on homepage - HomeIntro handles those with navigate()
