...
export function EquipmentItemCard({ item, viewMode, quantity, onQuantityChange, priority = false }: { 
  ...
  if (viewMode === 'list') {
    return (
      <div className={`group flex flex-row gap-x-4 items-center hover:bg-black/5 dark:hover:bg-natural/5 rounded py-2 px-4 transition-colors duration-md ease-es ${quantity > 0 ? 'bg-black/5 dark:bg-natural/5' : ''}`}>
        <div className="grow">
        ...
        <div className="flex flex-row gap-2 justify-center items-center">
          <button className={`qty opacity-0 ${quantity === 0 ? 'group-hover:opacity-60' : 'group-hover:opacity-100'}`} onClick={() => onQuantityChange(Math.max(0, quantity - 1))}>
            <Icon name="icon-minus" className="w-3 h-3 fill-natural dark:fill-black" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 14 14"><title>Decrease quantity</title></Icon>
          </button>
          ...
          <button className="qty opacity-0 group-hover:opacity-100" onClick={() => onQuantityChange(quantity + 1)}>
            <Icon name="icon-plus" className="w-3 h-3 fill-natural dark:fill-black" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 14 14"><title>Increase quantity</title></Icon>
          </button>
        </div>
        <div>
  ...
  return (
    <div className="flex flex-row gap-x-4">
      ...
      <div className="grow flex flex-row gap-x-4 pr-4 py-2">
        <div className="grow">
