...
export function StudioTerms({ onClose, studio, global }: Props) {
  ...
  return (
    <div className="grow flex flex-col pb-4 pt-20 px-2 lg:px-0 min-h-0">
      ...
      <div className="grid_ gap-y-md overflow-y-scroll">
        ...
        <div className="col-start-1 col-span-12 sm:col-start-2 sm:col-span-10 lg:col-start-6 lg:col-span-11 flex flex-col gap-y-6">
            {studio?.termsTitle && (
              <h3 className="label">{studio.termsTitle}</h3>
            )}

            {/* Intro */}
            {studio?.termsIntro && (
              <div className="terms">
                <PortableText value={studio.termsIntro} />
              </div>
            )}

            {/* Terms & Conditions Accordion */}
            {studio?.termsAndConditions && studio.termsAndConditions.length > 0 && (
              <ol className="terms">
                {studio.termsAndConditions.map((item, index) => (
                  <li key={index}>
                    <button onClick={() => toggleAccordion(index)} className="flex flex-row justify-between items-center w-full">
